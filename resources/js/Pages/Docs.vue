...
// Dynamic docs list from API
const allDocs = ref([]);
const fetchNavigation = async () => {
    try {
        const response = await axios.get('/docs/api/navigation');
        if (response.data && response.data.success) {
            allDocs.value = response.data.data;
        }
    } catch (e) {
        console.error("Failed to load documentation navigation", e);
    }
};
...
const onEditorSave = () => {
    // Automatically trigger save when Tiptap signals a save action.
    saveDoc();
};
...
const saveDoc = async () => {
    isSaving.value = true;
    try {
        // Tiptap V-Models HTML. We must convert it back to Markdown before saving.
        const td = new TurndownService({
            headingStyle: 'atx',
            codeBlockStyle: 'fenced'
        });
        const markdownContent = td.turndown(editableContent.value);
        ...
        // We do NOT reload here during autosave, Tiptap holds the state.
        // We only reload when the user explicitly cancels or navigates.
    } catch (e) {
        console.error(e);
                ...
                <main class="flex-1 flex flex-col bg-slate-50 dark:bg-[#0F172A] relative transition-all duration-300 overflow-hidden border-l border-slate-100 dark:border-white/5">
                    ...
                    <div class="flex-1 overflow-y-auto custom-scrollbar">
                        <article class="w-full max-w-7xl px-8 py-10 md:px-12 pb-40 xl:pr-[280px]">
                            ...
                            <!-- Doc Header -->
                            <div class="mb-10 border-b border-slate-100 dark:border-white/5 pb-8 flex items-start justify-between">
                                <div>
                                ...
                                <!-- Edit Button -->
                                <button v-if="!isEditing" @click="isEditing = true" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
                                    <span class="material-symbols-outlined text-[18px]">edit</span>
                                    Edit
                                </button>
                                <div v-else class="flex gap-2">
                                    <button @click="isEditing = false; editableContent = props.content" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium transition-colors">
                                        Cancel
                                    </button>
                                    <button @click="saveDoc" :disabled="isSaving" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2 disabled:opacity-50">
                            ...
                            <!-- Doc Content -->
                            <div v-if="!isEditing" class="prose prose-slate dark:prose-invert max-w-none prose-headings:font-sans prose-headings:font-bold prose-p:text-slate-600 dark:prose-p:text-slate-300 prose-a:text-indigo-500 hover:prose-a:text-indigo-600" v-html="renderedHtml" @click="handleContentClick"></div>
                            
                            <!-- Edit Mode (Tiptap Integration) -->
                            <div v-else class="flex flex-col gap-4 relative min-h-[600px] border border-slate-200 dark:border-white/10 rounded-xl overflow-hidden bg-white dark:bg-[#0b1121] shadow-inner">
                                <!-- NoteEditor provides its own toolbar and content area -->
                                <NoteEditor 
                                    v-model="editableContent" 
                                    :save-status="isSaving ? 'Saving...' : (lastSavedAt ? 'Saved' : '')"
